...
def compileCompiler():
  ...
  returnCode = subprocess.Popen("cd %s && javac Compiler.java" % srcPath, shell=True, stdout=subprocess.PIPE,
...
def deleteCompilationOutputs():
  outputExtensions = [".exe", ".o", ".out", ".sa", ".sc", ".ts", ".nasm", ".pre-nasm", ".c3a", ".c3aout", ".fg", ".fgs",
                      ".ig"]
  for filename in os.listdir(inputPath):
...
def compileInputFiles(inputFiles):
  for inputFile in inputFiles:
    ...
    returnCode = subprocess.Popen("{} {}{}".format(compiler(), inputPath, inputFile), shell=True,
...
def evaluateSa(inputFiles):
  ...
  for filename in inputFiles:
    ...
    res = subprocess.Popen("{} {} {}{}".format(compareArbres, saRef, inputPath, saFilename), shell=True,
...
def evaluateDiff(inputFiles, extension, path, name):
  ...
  for filename in inputFiles:
    ...
    res = subprocess.Popen("diff {} {}{}".format(ref, inputPath, producedFile), shell=True, stdout=subprocess.PIPE,
...
def evaluateNasm(inputFiles):
  for filename in inputFiles:
    ...
    out = subprocess.Popen("cd {} && nasm -f elf -dwarf -g {}".format(inputPath + "..", "input/" + nasmFilename),
                           shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stderr.read()
    if not os.path.isfile(inputPath + objectFilename):
      print(out, file=sys.stderr)
      continue
    out = subprocess.Popen("ld -m elf_i386 -o {}{} {}{}".format(inputPath, execFilename, inputPath, objectFilename),
                           shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stderr.read()
    if not os.path.isfile(inputPath + execFilename):
      print(out, file=sys.stderr)
      continue
    out = subprocess.Popen("{}{} > {}{}".format(inputPath, execFilename, inputPath, outFilename), shell=True,
                           stdout=subprocess.PIPE, stderr=subprocess.PIPE).stderr.read()
